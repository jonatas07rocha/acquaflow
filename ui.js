...
const modalTemplates = {
    ...
    achievementInfo: (ach) => `<div id="achievement-info-modal" class="modal-container fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-50"><div class="glass-panel p-6 rounded-2xl shadow-xl w-11/12 max-w-xs text-center"><div class="w-20 h-20 mx-auto mb-4 rounded-full flex items-center justify-center ${ach.u ? 'bg-amber-500/30' : 'bg-gray-500/20'}"><i data-lucide="${ach.u ? ach.icon : 'lock'}" class="w-10 h-10 ${ach.u ? 'text-amber-300' : 'text-gray-400'}"></i></div><h2 class="text-xl font-bold">${ach.n}</h2><p class="text-sm opacity-60 mb-1">${ach.u ? 'Desbloqueada!' : 'Bloqueada'}</p><p class="text-sm opacity-80 mb-6">${ach.desc}</p><button data-action="closeModal" class="w-full main-add-button text-white font-bold py-3 rounded-lg">Fechar</button></div></div>`,
    ...
    reminderConfirmation: () => `
        <div id="reminder-confirmation-modal" class="modal-container fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-50">
            <div class="glass-panel p-6 rounded-2xl shadow-xl w-11/12 max-w-xs text-center">
                <h2 class="text-xl font-bold mb-4">Lembretes Ativados!</h2>
                <p class="text-sm opacity-80 mb-6">Tudo certo! Se você não registrar um consumo por mais de uma hora, enviaremos uma notificação para te ajudar a se manter hidratado.</p>
                <button data-action="closeModal" class="w-full main-add-button text-white font-bold py-3 rounded-lg">Entendido</button>
            </div>
        </div>
    `,
...
export function showAchievementInfoModal(achievement) { closeAllModals(); document.body.insertAdjacentHTML('beforeend', modalTemplates.achievementInfo(achievement)); lucide.createIcons(); }
// NOVA FUNÇÃO PARA EXIBIR O MODAL DE CONFIRMAÇÃO
export function showReminderConfirmationModal() { closeAllModals(); document.body.insertAdjacentHTML('beforeend', modalTemplates.reminderConfirmation()); lucide.createIcons(); }
