...
const widgetTemplates = {
    progress: (state) => `
        <section data-widget-id="progress" class="widget glass-panel rounded-3xl p-6 my-4 flex flex-col items-center animate-on-scroll">
            <i data-lucide="grip-vertical" class="drag-handle"></i>
            <div class="relative w-56 h-56">
                <svg class="w-full h-full" viewBox="0 0 120 120">
                    <circle cx="60" cy="60" r="54" fill="none" stroke="rgba(255, 255, 255, 0.15)" stroke-width="12"/>
            ...
            <button data-action="showAddWater" class="main-add-button text-white font-bold py-4 w-full mt-6 rounded-full flex items-center justify-center mx-auto">
                <i data-lucide="droplet" class="w-5 h-5 mr-2"></i>Adicionar Água
            </button>
        </section>`,
    stats: (state) => `
        <div data-widget-id="stats" class="widget grid grid-cols-2 gap-4 mb-4 animate-on-scroll">
            <i data-lucide="grip-vertical" class="drag-handle"></i>
            <div class="glass-panel rounded-2xl p-4 flex items-center">
                <div class="p-3 rounded-full mr-4 bg-[color-mix(in_srgb,var(--color-accent)_30%,transparent)]">
                    <i data-lucide="target" class="w-5 h-5 text-[var(--color-accent-light)]"></i>
                </div>
                <div>
                    <div class="text-xs opacity-70">Meta Diária</div>
                    <div id="goal-text" class="text-lg font-semibold">${state.settings.dailyGoal} ml</div>
                </div>
            </div>
            <div class="glass-panel rounded-2xl p-4 flex items-center">
                <div class="p-3 rounded-full mr-4 bg-[color-mix(in_srgb,var(--color-accent)_30%,transparent)]">
                    <i data-lucide="hourglass" class="w-5 h-5 text-[var(--color-accent-light)]"></i>
                </div>
                <div>
    ...
    history: (state) => `
        <section data-widget-id="history" class="widget glass-panel rounded-2xl p-4 mb-4 animate-on-scroll relative">
            <i data-lucide="grip-vertical" class="drag-handle"></i>
            <div class="flex items-center mb-3">
                <div class="p-2 rounded-full mr-3 bg-[color-mix(in_srgb,var(--color-accent)_30%,transparent)]">
                    <i data-lucide="history" class="w-4 h-4 text-[var(--color-accent-light)]"></i>
                </div>
    ...
    weekly: (state) => `
        <section data-widget-id="weekly" class="widget glass-panel rounded-2xl p-4 mb-4 animate-on-scroll relative">
            <i data-lucide="grip-vertical" class="drag-handle"></i>
            <div class="flex items-center mb-4">
                <div class="p-2 rounded-full mr-3 bg-[color-mix(in_srgb,var(--color-accent)_30%,transparent)]">
                    <i data-lucide="bar-chart-2" class="w-4 h-4 text-[var(--color-accent-light)]"></i>
                </div>
    ...
    achievements: (state) => `
        <section data-widget-id="achievements" class="widget glass-panel rounded-2xl p-4 mb-4 animate-on-scroll relative">
            <i data-lucide="grip-vertical" class="drag-handle"></i>
            <div class="flex items-center mb-4">
                <div class="p-2 rounded-full mr-3 bg-amber-500/30">
    ...
    tip: () => `
        <section data-widget-id="tip" class="widget glass-panel rounded-2xl p-4 mb-4 animate-on-scroll relative">
            <i data-lucide="grip-vertical" class="drag-handle"></i>
            <div class="flex items-center mb-2">
                <div class="p-2 rounded-full mr-3 bg-lime-500/30">
                    <i data-lucide="lightbulb" class="w-4 h-4 text-lime-300"></i>
                </div>
...
const modalTemplates = {
    addWater: () => `
        <div id="add-water-modal" class="modal-container fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-50">
            <div class="glass-panel p-6 rounded-2xl shadow-xl w-11/12 max-w-xs text-center">
                <h2 class="text-xl font-bold mb-4">Adicionar Água</h2>
                <p class="text-sm opacity-80 mb-4">Digite a quantidade em ml.</p>
                <input type="number" id="custom-amount" class="w-full border-2 border-white/20 bg-white/10 rounded-lg p-3 text-center text-2xl text-white" placeholder="250">
                <div class="flex gap-3 mt-6">
                    <button data-action="closeModal" class="w-full bg-white/10 font-semibold py-3 rounded-lg hover:bg-white/20 transition-colors">Cancelar</button>
                    <button data-action="addCustomWater" class="w-full main-add-button text-white font-bold py-3 rounded-lg">Confirmar</button>
                </div>
            </div>
        </div>`,
    settings: (state) => `
        <div id="settings-modal" class="modal-container fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-50">
            <div class="glass-panel p-6 rounded-2xl shadow-xl w-11/12 max-w-sm text-left">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-xl font-bold">Configurações</h2>
                    <button data-action="closeModal" class="p-1 rounded-full hover:bg-white/20"><i data-lucide="x" class="w-5 h-5"></i></button>
                </div>
                <div class="space-y-6">
                    ...
                    <div class="flex justify-between items-center">
                        <div><h3 class="font-semibold">Lembretes</h3><p class="text-xs opacity-70">Receber notificações para beber água.</p></div>
                        <label class="switch"><input type="checkbox" id="reminders-toggle" ${state.settings.reminders ? 'checked' : ''}><span class="slider"></span></label>
                    </div>
                    <div>
                ...
                <button data-action="saveSettings" class="w-full main-add-button text-white font-bold py-3 mt-8 rounded-lg">Salvar e Fechar</button>
            </div>
        </div>`
};
...
function updateProgressCircle(state) {
